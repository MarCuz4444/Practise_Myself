...
func (service *serviceGolang) AddGolang(customer entity.Golang) int {
	...
	query := "INSERT INTO `customer`(id, name, age, phone) values($1,$2,$3,$4) returning customer_id"
	id := 0
	err = db.QueryRow(query, customer.ID, customer.Name, customer.Age, customer.Phone).Scan(&id)
	if err != nil {
		panic(err.Error())
	}
	return id
